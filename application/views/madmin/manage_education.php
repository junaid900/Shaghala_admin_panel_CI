<div class="row wrapper page-heading">
    <div class="col-md-12">
    	...
    	<div class="row col-md-12">
         ...
         <div class="">
            <button class="btn btn-primary" onclick="location.href='<?=base_url()."".admin_ctrl()."/add_education"?>'"><i class="fa fa-plus"></i>&nbsp;<?= $page_title ?></button>
        </div>	
    </div>
                ...
                <div class="">
                    <div class="table-responsive">
                        <table class="dataTables-example table-striped">
                            ...
                            <tr>
                                <th><?="No."?></th>
                                <th><?=get_phrase("en_degree")?></th>
                                <th><?=get_phrase("arb_degree")?></th>
                            ...
                            foreach ($table_data as $data) {$i++; ?>
                                <tr class="gradeX row_<?php echo $data['status']; ?>"
                                    id="<?php echo $data['edu_id']; ?>">
                                    <td><?= $i ?></td>
                                    <td><?= $data['en_degree'] ?></td>
                                    <td><?= $data['ar_degree'] ?></td>
                                    <td>
                                        <div class="table-toggle">
                                            <div class="toggle-btn1 <?php if ($data['status'] == 'Active') {
                                                ...
                                                <input type="checkbox" class="cb-value"
                                                       value="<?php echo $data['edu_id']; ?>" <?php if ($data['status'] == 'Active') {
                                                    echo 'checked';
                                                } ?>/>
                                    ...
                                    <td class="center">
                                        <a href="<?=base_url().admin_ctrl().'/edit_education/'.$data['edu_id']; ?>">
                                            <img src="<?=base_url(); ?>assets/admin/img/edit.png" class="mr_1" /> 
                                        </a>
                                        <a href="javascript:;"
                                           onclick="confirm_modal_action('<?php echo base_url() . admin_ctrl() ?>/<?= $page_name ?>/delete/<?php echo $data['edu_id']; ?>');">
